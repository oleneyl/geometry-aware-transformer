...
class Trainer:
    ...
    @tf.function
    def train_on_batch(self, dataset_iter, metrics, from_tfrecord=False):
        if from_tfrecord:
    ...
    @tf.function
    def test_on_batch(self, dataset_iter, metrics, from_tfrecord=False):
        if from_tfrecord:
            inputs = next(dataset_iter)
            targets = inputs['target']
        else:
    ...
    @tf.function
    def predict_on_batch_with_target(self, dataset_iter, metrics):
        inputs, targets = next(dataset_iter)
        preds = self.model(inputs, training=False)
    ...
    @tf.function
    def predict_only(self, dataset_iter):
        inputs, targets = next(dataset_iter)
        preds = self.model(inputs, training=False)
